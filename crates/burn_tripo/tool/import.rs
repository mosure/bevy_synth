...
fn asset_path(relative: impl AsRef<Path>) -> PathBuf {
    project_dir().join(relative)
}

fn default_dino_output() -> PathBuf {
    asset_path("assets/models/dinov2")
}

fn default_pca_weights() -> PathBuf {
    asset_path("assets/models/face_pca.safetensors")
}

fn default_pca_output() -> PathBuf {
    asset_path("assets/models/pca")
}
...
pub struct DinoImportConfig {
    ...
    #[arg(long, default_value_os_t = default_dino_output())]
    pub output: PathBuf,
    ...
    #[arg(long, default_value_os_t = default_pca_weights())]
    pub pca_weights: PathBuf,

    #[arg(long, default_value_os_t = default_pca_output())]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.validate {
        let reference_path = args.reference.clone().unwrap_or_else(|| {
            asset_path("assets/correctness").join(args.vit_type.reference_file())
        });
...
fn import_dino_weights(
    ...
    let weights_path = args
        .weights_path
        .clone()
        .unwrap_or_else(|| asset_path("assets/models").join(args.vit_type.weights_file()));
