...
fn triposg_vae_hooks_match_reference() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let weights_path = resolve_weights_path("vae/diffusion_pytorch_model.safetensors");
    if !weights_path.exists() {
        eprintln!(
            "skipping: VAE weights file not found at {}",
    ...
    let coords = reference
        ...
        .ok_or("missing input.coords in reference")?;
    let features = reference
        .get_input("input.features")
        .ok_or("missing input.features in reference")?;
    let query_coords = reference
        .get_input("input.query_coords")
        .ok_or("missing input.query_coords in reference")?;

    let coords = tensor_from_data::<burn::backend::NdArray<f32>>(&coords, &device)?;
    let features = tensor_from_data::<burn::backend::NdArray<f32>>(&features, &device)?;
    let query_coords = tensor_from_data::<burn::backend::NdArray<f32>>(&query_coords, &device)?;

    let mut hooks = HookRecorder::new();
    let _output = model.forward(coords, features, query_coords, true, Some(&mut hooks));
