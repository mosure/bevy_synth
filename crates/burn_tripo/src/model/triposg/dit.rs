...
pub mod import {
    ...
    pub fn import_triposg_dit_burnpack<B: Backend>(
        ...
        import_triposg_dit_burnpack_with_precision::<B>(config, device, path, precision)
    }

    pub fn import_triposg_dit_burnpack_with_precision<B: Backend>(
        config: &TripoSGDiTConfig,
        device: &B::Device,
        path: impl AsRef<Path>,
        ...
        let model = load_triposg_dit_import_source::<B>(config, device, path, default_policy)?;
        let model = apply_import_precision(model, precision);
        save_burnpack(&model, &burnpack_path)?;
        Ok(burnpack_path)
    }

    fn load_triposg_dit_import_source<B: Backend>(
        config: &TripoSGDiTConfig,
        device: &B::Device,
        path: &Path,
        policy: BurnpackLoadPolicy,
        ...
        if path
            ...
            && path.exists()
        {
            return load_triposg_dit_from_safetensors::<B>(config, device, path);
        }

        let f32_burnpack_path = burnpack_path(path, false, policy.f16_suffix);
        if f32_burnpack_path.exists() {
            return load_triposg_dit_from_burnpack_file(config, device, &f32_burnpack_path);
        }

        Err(format!(
            "missing TripoSG DiT source weights for import at {} (expected .safetensors or {})",
            path.display(),
