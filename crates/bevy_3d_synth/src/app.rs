...
use bevy_3d_synth_core::args::{AppArgs, Args, build_app_args};
use bevy_3d_synth_core::cache::{CachedWorldItem, MeshCache};
use bevy_3d_synth_core::io::{is_image_file, is_mesh_file, resolve_output_path, write_obj};
use bevy_3d_synth_core::mesh::to_bevy_mesh;
use bevy_3d_synth_core::state::{
    ExitState, InferenceQueue, InferenceRequest, InferenceWorker, Spinner, TitlePulse, UiStatus,
    WorkerCommand,
};
use bevy_3d_synth_core::worker::start_worker;

use crate::ui::{
    Burn3dSynthUiPlugin, CatalogState, CatalogUiState, DragState, preview_light_layers,
};

#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Debug)]
pub(crate) struct ImagePickDialog;
...
pub(crate) fn run() {
    ...
    app.add_plugins(TransformGizmoPlugin)
        ...
        .add_plugins(Burn3dSynthUiPlugin)
        .add_systems(Startup, (setup, configure_mesh_picking).chain())
        .add_systems(PostStartup, mark_gizmo_pick_targets)
        .add_systems(
            Update,
            (
...
fn web_asset_root() -> String {
    if let Some(value) = option_env!("BURN_3D_SYNTH_WEB_ASSET_ROOT") {
...
fn sync_panorbit_enabled(
    ...
    let ui_block = windows
        ...
        .unwrap_or(false);
    let enabled = !gizmo_active && !drag.is_dragging() && !ui_block;
    for mut camera in cameras.iter_mut() {
        if !enabled {
...
fn update_window_title(
    ...
    let title = if let Some(active) = queue.active.as_ref() {
        ...
        format!(
            "burn_3d_synth — processing: {name} (queued: {}){dots}",
            queue.pending.len()
        )
    } else {
        format!("burn_3d_synth — {}", status.message)
    };
    ...
    match result {
        Ok(Some(mesh)) => {
            ...
            if let Some(entry) = catalog.entry_mut(request.id) {
                entry.status = crate::ui::CatalogStatus::Ready;
        ...
        Ok(None) => {
            ...
            if let Some(entry) = catalog.entry_mut(request.id) {
                entry.status = crate::ui::CatalogStatus::Failed("empty mesh".to_string());
                catalog.bump_revision();
            }
        }
        Err(err) => {
            ...
            if let Some(entry) = catalog.entry_mut(request.id) {
                entry.status = crate::ui::CatalogStatus::Failed(err);
                catalog.bump_revision();
            }
        }
    }
