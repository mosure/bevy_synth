...
use crate::app::{MainCamera, spawn_mesh_instance};
use bevy_3d_synth_core::state::InferenceRequest;

#[cfg(not(target_arch = "wasm32"))]
use crate::app::ImagePickDialog;
#[cfg(not(target_arch = "wasm32"))]
...
pub(crate) struct Burn3dSynthUiPlugin;

impl Plugin for Burn3dSynthUiPlugin {
...
impl CatalogState {
    pub(crate) fn add_pending(&mut self, request: &InferenceRequest) {
        let label = request
            .image_path
            .file_name()
    ...
    pub(crate) fn entry(&self, id: u32) -> Option<&CatalogEntry> {
        self.entries.iter().find(|entry| entry.id == id)
    }

    pub(crate) fn entry_mut(&mut self, id: u32) -> Option<&mut CatalogEntry> {
    ...
    pub(crate) fn alloc_preview_layer(&mut self) -> Option<usize> {
        self.available_layers.pop()
    }

    pub(crate) fn release_preview_layer(&mut self, layer: usize) {
...
pub(crate) struct CatalogEntry {
    ...
    pub(crate) mesh: Option<Handle<BevyMesh>>,
    pub(crate) material: Option<Handle<StandardMaterial>>,
    pub(crate) source_image_path: Option<String>,
    pub(crate) cache_key: Option<String>,
    pub(crate) preview: Option<PreviewScene>,
}
...
#[derive(Resource)]
pub(crate) struct CatalogUiState {
    list_entity: Entity,
    last_revision: u64,
    last_expanded: bool,
    panel_width: f32,
...
impl CatalogUiState {
    pub(crate) fn cursor_over_ui(&self, window: &Window) -> bool {
        window
            .cursor_position()
            .map(|cursor| {
...
fn update_queue_text(
    queue: Res<bevy_3d_synth_core::state::InferenceQueue>,
...
pub(crate) fn preview_light_layers() -> RenderLayers {
