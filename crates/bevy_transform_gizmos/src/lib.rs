...
fn on_transform_gizmo_pointer_release(
    ...
    let Some(interaction) = gizmo.interaction else {
        return;
    };
    if interaction.pointer_id != trigger.pointer_id {
        return;
    }
    if let Some(from) = gizmo.initial_transform {
...
fn drag_gizmo(
    ...
    gizmo_cameras: Query<Entity, With<GizmoCamera>>,
) {
    ...
    {
        let preferred_camera = gizmo_cameras.single().ok();
        let ray = preferred_camera
            .and_then(|camera_entity| {
                raymap
                    .iter()
                    .find(|(id, _)| id.pointer == pointer_id && id.camera == camera_entity)
                    .map(|(_, ray)| *ray)
            })
            .or_else(|| {
                raymap
                    .iter()
                    .find(|(id, _)| id.pointer == pointer_id)
                    .map(|(_, ray)| *ray)
            });
