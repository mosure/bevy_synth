...
/// Default CUDA backend type used by the sparse patchify tensor wrapper.
pub type DefaultCudaBackend = CubeBackend<CudaRuntime, f32, i32, u8>;
...
pub fn sparse_patchify3d_forward_cuda(
    config: &SparsePatchify3dConfig,
    input: BurnTensor<DefaultCudaBackend, 5>,
    coords: BurnTensor<DefaultCudaBackend, 2, Int>,
    weight: BurnTensor<DefaultCudaBackend, 5>,
    bias: BurnTensor<DefaultCudaBackend, 1>,
) -> Result<BurnTensor<DefaultCudaBackend, 2>, String> {
    ...
    if rows == 0 {
        return Ok(BurnTensor::<DefaultCudaBackend, 2>::zeros(
    ...
    unsafe {
        sparse_patchify3d_kernel::launch_unchecked::<CudaRuntime>(
            &client,
            cube_count,
            cube_dim,
    ...
    Ok(BurnTensor::<DefaultCudaBackend, 2>::from_primitive(
...
mod tests {
    ...
    use super::{DefaultCudaBackend, sparse_patchify3d_forward_cuda};
    ...
    #[test]
    fn cuda_sparse_patchify3d_matches_cpu_flex_path() {
        if std::env::var("BURN_FLEX_GMM_RUN_CUDA_TEST").ok().as_deref() != Some("1") {
            eprintln!("skipping CUDA sparse patchify test; set BURN_FLEX_GMM_RUN_CUDA_TEST=1");
            return;
        }
        ...
        let device = burn_cubecl::cubecl::cuda::CudaDevice::default();
        let input_t = Tensor::<DefaultCudaBackend, 1>::from_floats(input.as_slice(), &device)
        ...
        let coords_flat: Vec<i64> = coords
            ...
            .collect();
        let coords_t = Tensor::<DefaultCudaBackend, 1, burn::tensor::Int>::from_data(
            TensorData::new(coords_flat, [coords.len() * 4]),
            &device,
        )
        .reshape([coords.len(), 4]);
        let weight_t = Tensor::<DefaultCudaBackend, 1>::from_floats(weight.as_slice(), &device)
            .reshape([
                cfg.out_channels,
                cfg.in_channels,
                cfg.tubelet_size,
        ...
        let bias_t = Tensor::<DefaultCudaBackend, 1>::from_floats(bias.as_slice(), &device);
        let output = sparse_patchify3d_forward_cuda(&cfg, input_t, coords_t, weight_t, bias_t)
            .expect("cuda sparse patchify path")
            .to_data();
