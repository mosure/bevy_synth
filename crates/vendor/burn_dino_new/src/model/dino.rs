...
impl<B: Backend> DinoVisionTransformer<B> {
    ...
    pub fn interpolate_pos_encoding(&self, x: Tensor<B, 3>, W: usize, H: usize) -> Tensor<B, 3> {
        ...
        let patch_pos_embed = self.interpolate_pos_embedding_bicubic(
            patch_pos_embed,
            M,
            dim,
            target_h.max(1),
    ...
    fn interpolate_pos_embedding_bicubic(
        &self,
        ...
        let hw_tokens = patch_pos_embed
            .swap_dims(1, 2)
            .reshape([1_i32, channels as i32, input_grid as i32, input_grid as i32]);
