...
impl TensorSnapshot {
    ...
    pub fn data_len(&self) -> usize {
        // QFloat tensor bytes include quantization params in addition to values.
        // shape * dtype.size() underestimates serialized length for this dtype.
        if matches!(self.dtype, burn_tensor::DType::QFloat(_)) {
            if let Ok(data) = self.to_data() {
