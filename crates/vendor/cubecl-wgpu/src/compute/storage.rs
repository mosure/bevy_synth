...
impl WgpuResource {
    ...
    pub fn as_wgpu_bind_resource(&self) -> wgpu::BindingResource<'_> {
        // WebGPU storage bindings must be 4-byte aligned. This is required on
        // wasm WebGPU runtimes for f16 tensors with odd element counts.
        let binding_size = align_storage_binding_size(self.size);
        let binding = wgpu::BufferBinding {
            buffer: &self.buffer,
            offset: self.offset,
            size: Some(
                NonZeroU64::new(binding_size).expect("0 size resources are not yet supported."),
            ),
        };
...
impl ComputeStorage for WgpuStorage {
    ...
    fn alloc(&mut self, size: u64) -> Result<StorageHandle, IoError> {
        ...
        let alloc_size = align_storage_binding_size(size);
...
#[inline]
fn align_storage_binding_size(size: u64) -> u64 {
    // Keep non-empty resources non-empty and round up to 4-byte boundary.
    if size == 0 {
        return 0;
    }
    (size + 3) & !3
