...
pub async fn download_burnpack_asset<F>(
...
async fn download_burnpack_from_manifest<F>(
    manifest_url: &str,
    label: &str,
    max_bytes: u64,
    totals: &mut DownloadTotals,
    ...
    F: FnMut(String),
{
    ...
    if manifest.total_bytes > usize::MAX as u64 {
        return Err(format!(
            "manifest {manifest_url} declares {} bytes which exceeds platform limit {}",
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        .await?;
        if let Some(expected_sha) = shard_expected_sha256(entry) {
            let actual_sha = sha256_hex(&bytes);
            if !actual_sha.eq_ignore_ascii_case(expected_sha) {
                return Err(format!(
                    "shard checksum mismatch for {shard_url}: expected {expected_sha}, got {actual_sha}"
                ));
            }
        }
        host_ram_budget.observe_temporary(
    ...
    on_status(format!(
        "Reconstructed {label} from {} shards ({} bytes)",
        entries.len(),
        output.len()
    ));
    if !manifest.sha256.trim().is_empty() {
        let actual_sha = sha256_hex(&output);
        if !actual_sha.eq_ignore_ascii_case(manifest.sha256.trim()) {
            return Err(format!(
                "manifest checksum mismatch for {manifest_url}: expected {}, got {}",
                manifest.sha256.trim(),
                actual_sha
            ));
        }
        on_status(format!("Verified checksum for {label}"));
    }
    Ok(output)
}

fn shard_expected_sha256(entry: &burn_synth_import::shard::BurnpackShardEntry) -> Option<&str> {
    match entry {
        burn_synth_import::shard::BurnpackShardEntry::Path(_) => None,
        burn_synth_import::shard::BurnpackShardEntry::Detailed { sha256, .. } => sha256
            .as_deref()
            .map(str::trim)
