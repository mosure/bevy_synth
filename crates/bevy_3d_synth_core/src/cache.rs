...
fn default_native_cache_root() -> PathBuf {
    if let Ok(explicit) = std::env::var("BURN_3D_SYNTH_CACHE_DIR") {
    ...
    PathBuf::from(".burn_3d_synth_cache")
}

#[cfg(target_arch = "wasm32")]
fn default_web_cache_prefix() -> String {
    if let Ok(explicit) = std::env::var("BURN_3D_SYNTH_CACHE_PREFIX") {
        let trimmed = explicit.trim();
        if !trimmed.is_empty() {
            return trimmed.to_string();
        }
    }
    if let Some(prefix) = option_env!("BURN_3D_SYNTH_CACHE_PREFIX") {
        let trimmed = prefix.trim();
        if !trimmed.is_empty() {
            return trimmed.to_string();
        }
    }
    "burn_3d_synth/cache/v1".to_string()
}
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let unique = format!(
            "burn_3d_synth_cache_test_{name}_{}_{}",
            std::process::id(),
            now_unix_ms()
        );
