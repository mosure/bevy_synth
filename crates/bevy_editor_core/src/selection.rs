...
fn selection_handler(
    ...
    let Some(target) = resolve_selectable_target(
...
mod tests {
    ...
    #[test]
    fn resolve_selectable_target_walks_parent_chain() {
        let mut world = World::new();
        let parent = world.spawn(Selectable).id();
