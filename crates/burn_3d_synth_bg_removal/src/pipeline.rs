...
fn is_valid_alpha(alpha: &[u8], width: usize, height: usize, min_ratio: f32) -> bool {
    let bins = 20usize;
...
fn find_bounding_box(mask: &[u8], width: usize, height: usize) -> Option<[usize; 4]> {
    let mut visited = vec![false; mask.len()];
    let mut queue = VecDeque::new();
...
fn bbox_from_mask_all(mask: &[u8], width: usize, height: usize) -> Option<[usize; 4]> {
    let mut min_x = width;
...
fn rgb_to_chw(rgb: &[u8], width: usize, height: usize) -> Vec<f32> {
    let pixels = width * height;
