...
impl<B: Backend> Attention<B> {
    ...
    pub fn forward(
        ...
        let use_module_attention = !self.quiet_softmax && attn_mask.is_none();
        let x = if use_module_attention {
            if attention_debug_enabled() && N >= 1024 {
                let backend_name = std::any::type_name::<B>();
                eprintln!(
                    "burn_dino: attn dispatch backend={backend_name} impl=flash_attention(module_attention) q={N} k={N} head_dim={}",
                    C / self.num_heads
                );
            }
            module_attention(
            ...
            if attention_debug_enabled() && N >= 1024 {
                let backend_name = std::any::type_name::<B>();
                let reason = if self.quiet_softmax {
                    "quiet_softmax"
                } else {
                    "attn_mask"
                };
                eprintln!(
                    "burn_dino: attn dispatch backend={backend_name} impl=dense_softmax reason={reason} q={N} k={N} head_dim={}",
                    C / self.num_heads
                );
            }
            ...
            attn.matmul(v).swap_dims(1, 2).reshape([B, N, C])
        };
...
fn attention_debug_enabled() -> bool {
    ...
    env_enabled("BURN_DINO_ATTN_DEBUG")
        || env_enabled("TRELLIS2_ATTN_DEBUG")
        || env_enabled("TRELLIS2_STAGE_DEBUG")
}
