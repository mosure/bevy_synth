...
impl<B: Backend> DinoVisionTransformer<B> {
    ...
    fn finalize_output(
        ...
        let b_dim = tokens.shape().dims::<3>()[0];
        let n_dim = tokens.shape().dims::<3>()[1];
    ...
    pub fn interpolate_pos_encoding(&self, x: Tensor<B, 3>, W: usize, H: usize) -> Tensor<B, 3> {
        let npatch = x.shape().dims::<3>()[1] - 1;
        let register_offset = self.register_token_count;
        let tokens_start = 1 + register_offset;
        let N = self.pos_embed.shape().dims::<3>()[1] - tokens_start;

        let b_dim = self.pos_embed.shape().dims::<3>()[0];
        let n_dim = self.pos_embed.shape().dims::<3>()[1];
        // let c_dim: usize = self.pos_embed.shape().dims::<3>()[2];
    ...
    pub fn prepare_tokens_with_masks(
        ...
        let x = Tensor::cat(
            vec![
                self.cls_token
                    .val()
                    .expand([x.shape().dims::<3>()[0] as i64, -1, -1]),
                x,
            ],
            1,
        );
        ...
        if let Some(register_tokens) = &self.register_tokens {
            let cls = x.clone().slice([0..x.shape().dims::<3>()[0], 0..1]);
            let patches = x
                .clone()
                .slice([0..x.shape().dims::<3>()[0], 1..x.shape().dims::<3>()[1]]);
    ...
    fn apply_camera_token(
        ...
        let batch = tokens.shape().dims::<3>()[0];
        let embed_dim = tokens.shape().dims::<3>()[2];
        ...
        let tail = tokens.clone().slice([
            0..batch as i32,
            1..tokens.shape().dims::<3>()[1] as i32,
            0..embed_dim as i32,
        ]);
    ...
    fn prepare_rope_positions(
        ...
        // HF DINOv3 rope coordinate calibration can differ across model variants.
        // Keep defaults matching current behavior, but allow controlled runtime
        // sweeps for parity debugging.
        let cfg = rope_debug_config();
        for y in 0..patches_h {
            for x in 0..patches_w {
                let y_pos = if cfg.use_patch_centers {
                    ((y as f32 + 0.5) / patches_h as f32) * 2.0 - 1.0
                } else {
                    (y as f32 / patches_h.max(1) as f32) * 2.0 - 1.0
                };
                let x_pos = if cfg.use_patch_centers {
                    ((x as f32 + 0.5) / patches_w as f32) * 2.0 - 1.0
                } else {
                    (x as f32 / patches_w.max(1) as f32) * 2.0 - 1.0
                };
                if cfg.swap_xy {
                    local.push(x_pos * cfg.scale);
                    local.push(y_pos * cfg.scale);
                } else {
                    local.push(y_pos * cfg.scale);
                    local.push(x_pos * cfg.scale);
                }
        ...
        let camera = if self.use_camera_tokens {
            Some(
                combined
                    .clone()
                    .slice([
                        0..combined.shape().dims::<3>()[0] as i32,
                        0..1,
                        0..combined.shape().dims::<3>()[2] as i32,
                    ])
                    .squeeze_dim(1),
            )
        } else {
...
#[derive(Clone, Copy)]
struct RopeDebugConfig {
    scale: f32,
    swap_xy: bool,
    use_patch_centers: bool,
}

fn rope_debug_config() -> RopeDebugConfig {
    static CONFIG: OnceLock<RopeDebugConfig> = OnceLock::new();
    *CONFIG.get_or_init(|| {
        let scale = std::env::var("BURN_DINO_ROPE_POS_SCALE")
            .ok()
            .and_then(|value| value.parse::<f32>().ok())
            .filter(|value| value.is_finite())
            .unwrap_or(2.0 * std::f32::consts::PI);
        let swap_xy = std::env::var("BURN_DINO_ROPE_SWAP_XY")
            .ok()
            .map(|value| {
                matches!(
            ...
            .unwrap_or(false);
        let use_patch_centers = std::env::var("BURN_DINO_ROPE_PATCH_CENTERS")
            .ok()
            .map(|value| {
                matches!(
                    value.trim().to_ascii_lowercase().as_str(),
